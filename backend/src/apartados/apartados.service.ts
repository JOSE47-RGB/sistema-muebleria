...
export class ApartadosService {
    ...
    private validarId(id: number, mensaje = 'Identificador inválido') {
    ...
    private normalizarToken(token?: string) {
        ...
        if (!tokenNormalizado ||
            ...
            throw new BadRequestException('El token de la operación es inválido');
        }
        return tokenNormalizado;
    }
    private validarFecha(fecha: string | null | undefined, nombre: string) {
        const valor = fecha?.trim();
        if (!valor ||
            !/^\d{4}-\d{2}-\d{2}$/.test(valor)) {
            throw new BadRequestException(`${nombre} es inválida`);
        }
        const fechaInterpretada = new Date(`${valor}T00:00:00`);
        if (Number.isNaN(fechaInterpretada.getTime())) {
            throw new BadRequestException(`${nombre} es inválida`);
        }
        return valor;
    }
    private async obtenerConfiguracionNumero(clave: string, valorPredeterminado: number) {
        const configuraciones: any[] = await this.prisma.$queryRaw `
      SELECT valor
    ...
    `;
        ...
        const valor = Number(configuraciones[0].valor);
        return Number.isFinite(valor)
            ? valor
            : valorPredeterminado;
    }
    private async obtenerContextoUsuario(idUsuario: number): Promise<ContextoUsuario> {
        this.validarId(idUsuario, 'Identificador de usuario inválido');
      ...
      `;
        if (usuarios.length === 0) {
            throw new BadRequestException('El usuario no tiene una sucursal activa asignada');
        }
        return {
    ...
    private async obtenerTurnoActivo(idUsuario: number, idSucursal: number): Promise<TurnoActivo> {
        const turnos: any[] = await this.prisma.$queryRaw `
        SELECT
          ct.id_turno,
          ct.id_caja,
          c.nombre AS caja
      ...
      `;
        if (turnos.length === 0) {
            throw new BadRequestException('Debes abrir caja antes de registrar apartados o abonos');
        }
        return {
    ...
    async catalogos(idUsuario: number) {
        const usuario = await this.obtenerContextoUsuario(idUsuario);
        ...
        try {
            turno = await this.obtenerTurnoActivo(idUsuario, usuario.id_sucursal);
        }
        catch {
            turno = null;
        }
      ...
      `;
        const productos: any[] = await this.prisma.$queryRaw `
        SELECT
          pv.id_variante,
          p.id_producto,
          p.codigo_producto,
      ...
      `;
        const metodosPago: any[] = await this.prisma.$queryRaw `
        SELECT
          id_metodo_pago,
          codigo,
          nombre,
      ...
      `;
        return {
            ...
            turno,
            politica: {
                porcentaje_minimo_entrega: porcentajeMinimoEntrega,
            ...
            clientes: clientes.map((cliente) => ({
                ...
                porcentaje_descuento: Number(cliente.porcentaje_descuento),
            })),
            productos: productos.map((producto) => ({
                ...
                stock_reservado: Number(producto.stock_reservado),
                stock_disponible: Number(producto.stock_disponible),
            })),
            metodos_pago: metodosPago.map((metodo) => ({
                id_metodo_pago: Number(metodo.id_metodo_pago),
                codigo: metodo.codigo,
                nombre: metodo.nombre,
                requiere_referencia: Number(metodo.requiere_referencia),
            })),
        };
    }
    async listar(idUsuario: number, filtros: FiltrosApartados) {
        const usuario = await this.obtenerContextoUsuario(idUsuario);
        let sql = `
      SELECT
        a.id_apartado,
    ...
    `;
        ...
        if (filtros.estado?.trim()) {
            sql += ` AND es.codigo = ?`;
            parametros.push(filtros.estado.trim().toUpperCase());
        }
      ...
      `;
            parametros.push(buscar, buscar, buscar, buscar, buscar);
        }
        sql += `
      ORDER BY
        CASE es.codigo
      ...
      LIMIT 300
    `;
        const apartados: any[] = await this.prisma.$queryRawUnsafe(sql, ...parametros);
        return apartados.map((apartado) => ({
            ...
            saldo_pendiente: Number(apartado.saldo_pendiente),
            total_pagado: Number(apartado.total) - Number(apartado.saldo_pendiente),
            porcentaje_pagado: Number(apartado.porcentaje_pagado),
            entregado: Number(apartado.entregado),
            fecha_entrega: apartado.fecha_entrega,
            elegible_entrega: Number(apartado.elegible_entrega),
            codigo_estado: apartado.codigo_estado,
            estado: apartado.estado,
            codigo_cliente: apartado.codigo_cliente,
            cliente: apartado.cliente,
            usuario: apartado.usuario,
        }));
    }
    async obtener(idApartado: number, idUsuario: number) {
        this.validarId(idApartado, 'Identificador de apartado inválido');
        const usuario = await this.obtenerContextoUsuario(idUsuario);
        const apartados: any[] = await this.prisma.$queryRaw `
        SELECT
          a.id_apartado,
          a.codigo_apartado,
          a.id_cliente,
      ...
      `;
        ...
        const detalles: any[] = await this.prisma.$queryRaw `
        SELECT
      ...
      `;
        const pagos: any[] = await this.prisma.$queryRaw `
        SELECT
          pg.id_pago,
          pg.token_operacion,
          pg.id_metodo_pago,
      ...
      `;
        ...
        return {
            ...
            detalles: detalles.map((detalle) => ({
                id_apartado_detalle: Number(detalle.id_apartado_detalle),
    ...
    private async buscarPorToken(tokenOperacion: string, idUsuario: number) {
        const apartados: any[] = await this.prisma.$queryRaw `
        SELECT id_apartado
        FROM apartados
      ...
      `;
        ...
        return this.obtener(Number(apartados[0].id_apartado), idUsuario);
    }
    async crear(data: CreateApartadoDto, idUsuario: number) {
        const tokenOperacion = this.normalizarToken(data.token_operacion);
        const existente = await this.buscarPorToken(tokenOperacion, idUsuario);
        if (existente) {
            return {
                mensaje: 'El apartado ya había sido registrado',
                apartado: existente,
            };
        }
        ...
        this.validarId(idCliente, 'Debes seleccionar un cliente');
        this.validarId(idMetodoPago, 'Debes seleccionar un método de pago');
        if (!Number.isFinite(enganche) ||
            enganche < 0) {
            throw new BadRequestException('El enganche no puede ser negativo');
        }
        if (!Array.isArray(data.detalles) ||
            data.detalles.length === 0) {
            throw new BadRequestException('El apartado debe contener productos');
        }
        const cantidadCuotas = Number(data.cantidad_cuotas);
        const frecuenciaPago = data.frecuencia_pago?.trim().toUpperCase();
        const fechaPrimerPago = this.validarFecha(
            data.fecha_primer_pago,
            'La fecha del primer pago',
        );

        if (!Number.isInteger(cantidadCuotas) ||
            cantidadCuotas <= 0 ||
            cantidadCuotas > 24) {
            throw new BadRequestException(
                'La cantidad de cuotas debe estar entre 1 y 24',
            );
        }
        ...
        const detallesAgrupados = new Map<number, number>();
        for (const detalle of data.detalles) {
            const idVariante = Number(detalle.id_variante);
            const cantidad = Number(detalle.cantidad);
            if (!Number.isInteger(idVariante) ||
                idVariante <= 0 ||
                !Number.isInteger(cantidad) ||
                cantidad <= 0) {
                throw new BadRequestException('Los productos y cantidades deben ser válidos');
            }
            detallesAgrupados.set(idVariante, (detallesAgrupados.get(idVariante) || 0) +
                cantidad);
        }
        ...
        try {
            const idApartado = await this.prisma.$transaction(async (tx) => {
                const cliente: any[] = await tx.$queryRaw `
                SELECT id_cliente
                FROM clientes
              ...
              `;
                ...
                const metodoPago: any[] = await tx.$queryRaw `
                SELECT
                  id_metodo_pago,
                  requiere_referencia
                FROM metodos_pago
              ...
              `;
                if (estadoActivo.length === 0) {
                    throw new BadRequestException('No existe el estado APARTADO/ACTIVO');
                }
                const tipoReserva: any[] = await tx.$queryRaw `
              ...
              `;
                if (tipoReserva.length === 0) {
                    throw new BadRequestException('No existe el movimiento RESERVA_APARTADO');
                }
                const tipoCajaAbono: any[] = await tx.$queryRaw `
              ...
              `;
                ...
                let total = 0;
                for (const [idVariante, cantidad,] of detallesAgrupados.entries()) {
                    const productos: any[] = await tx.$queryRaw `
                  SELECT
                    pv.id_variante,
                    pv.precio_venta,
                    i.stock_actual,
                ...
                `;
                    if (productos.length === 0) {
                        throw new BadRequestException(`La variante ${idVariante} no está disponible`);
                    }
                    const stockActual = Number(productos[0].stock_actual);
                    const stockReservado = Number(productos[0].stock_reservado);
                    const disponible = stockActual - stockReservado;
                    if (disponible < cantidad) {
                        throw new BadRequestException(`Stock insuficiente para la variante ${idVariante}. Disponible: ${disponible}`);
                    }
                    const precio = Number(productos[0].precio_venta);
                ...
                if (total <= 0) {
                    throw new BadRequestException('El total del apartado es inválido');
                }
                if (enganche > total) {
                    throw new BadRequestException('El enganche no puede superar el total');
                }
                const saldoPendiente = Number((total - enganche).toFixed(2));
                const codigoTemporal = `TMP-${tokenOperacion}`;
                await tx.$executeRaw `
              INSERT INTO apartados (
                ...
                ${idUsuario},
                ${turno.id_turno},
                ${Number(estadoActivo[0].id_estado)},
                NOW(),
                ${total},
                ${enganche},
                ${saldoPendiente},
            ...
            `;
                ...
                SELECT LAST_INSERT_ID()
                       AS id_apartado
              `;
                const nuevoId = Number(resultadoId[0]?.id_apartado);
                if (!Number.isInteger(nuevoId) ||
                    nuevoId <= 0) {
                    throw new BadRequestException('No fue posible crear el apartado');
                }
                const codigoApartado = `APA${String(nuevoId).padStart(6, '0')}`;
                await tx.$executeRaw `
              UPDATE apartados
              SET codigo_apartado =
                  ${codigoApartado}
            ...
            `;
              ...
              `;
                ...
                `;
                    if (Number(actualizados) !== 1) {
                        throw new BadRequestException('El inventario cambió durante la operación. Intenta nuevamente');
                    }
                    await tx.$executeRaw `
              ...
              `;
                ...
                await this.generarDocumentoApartadoTx(tx, {
                    idApartado: nuevoId,
                    codigoApartado,
                    idCliente,
                    idUsuario,
            ...
            const existenteDespues = await this.buscarPorToken(tokenOperacion, idUsuario);
            if (existenteDespues) {
                return {
                    mensaje: 'El apartado ya había sido registrado',
    ...
    async entregar(idApartado: number, idUsuario: number) {
        ...
        await this.prisma.$transaction(async (tx) => {
          ...
          `;
            ...
            if (Number(apartado.entregado) === 1) {
                throw new BadRequestException('Los productos ya fueron entregados');
            }
            if (apartado.codigo_estado !== 'ACTIVO') {
                throw new BadRequestException('El apartado no está activo');
            }
            const total = Number(apartado.total);
            const saldo = Number(apartado.saldo_pendiente);
            const porcentaje = ((total - saldo) / total) * 100;
            if (porcentaje < 85) {
                throw new BadRequestException(`El cliente ha pagado ${porcentaje.toFixed(2)}%. Debe alcanzar al menos el 85%`);
            }
          ...
          `;
            const tipoSalida: any[] = await tx.$queryRaw `
            SELECT id_tipo_movimiento
            FROM tipos_movimiento_inventario
          ...
          `;
            if (tipoSalida.length === 0) {
                throw new BadRequestException('No existe el movimiento SALIDA_VENTA');
            }
            ...
            `;
                if (Number(actualizados) !== 1) {
                    throw new BadRequestException('El inventario reservado no es suficiente para completar la entrega');
                }
                await tx.$executeRaw `
            INSERT INTO inventario_movimientos (
              id_sucursal,
        ...
        `;
             ...
             */
            if (saldo === 0) {
                const estadoCompletado: any[] = await tx.$queryRaw `
              SELECT id_estado
              FROM estados_sistema
              WHERE modulo = 'APARTADO'
            ...
            `;
                if (estadoCompletado.length === 0) {
                    throw new BadRequestException('No existe el estado APARTADO/COMPLETADO');
                }
                await tx.$executeRaw `
            UPDATE apartados
            SET id_estado =
                ${Number(estadoCompletado[0].id_estado)}
            WHERE id_apartado =
                  ${idApartado}
          `;
            }
    ...
    `;
        if (tipo.length === 0 ||
            estado.length === 0) {
            throw new BadRequestException('No existe la configuración para generar el vale');
        }
        const idTipo = Number(tipo[0].id_tipo_documento);
        const codigoDocumento = await this.siguienteDocumentoTx(tx, data.idSucursal, idTipo, tipo[0].prefijo || 'VAL');
        await tx.$executeRaw `
    ...
    `;
        const secuencias: any[] = await tx.$queryRaw `
        SELECT
          id_secuencia,
          serie,
          correlativo_actual
      ...
      `;
        if (secuencias.length === 0) {
            throw new BadRequestException('No existe una secuencia documental activa');
        }
        const correlativo = Number(secuencias[0].correlativo_actual) + 1;
        await tx.$executeRaw `
      UPDATE secuencias_documentos
      SET correlativo_actual =
          ${correlativo}
      WHERE id_secuencia =
            ${Number(secuencias[0].id_secuencia)}
    `;
        return `${prefijo}-${secuencias[0].serie}-${String(correlativo).padStart(6, '0')}`;
    }
    private manejarError(error: unknown): never {
        ...
        const mensaje = String(errorDb.meta?.message ||
            errorDb.message ||
            '');
        if (mensaje.includes('uk_apartados_token_operacion')) {
            throw new BadRequestException('El apartado ya fue procesado');
        }
        if (mensaje.includes('uk_pagos_token_operacion')) {
            throw new BadRequestException('El abono ya fue procesado');
        }
        if (mensaje.includes('uk_apartado_detalle_variante')) {
            throw new BadRequestException('Una variante está repetida en el apartado');
        }
        if (mensaje.includes('chk_apartados') ||
            mensaje.includes('chk_apartado_detalle')) {
            throw new BadRequestException('Los totales o cantidades del apartado son inválidos');
        }
        if (mensaje.includes('sp_generar_cuotas_apartado') &&
            mensaje.includes('does not exist')) {
            throw new BadRequestException(
                'No existe el procedimiento sp_generar_cuotas_apartado en la base de datos',
            );
        }
        if (mensaje.includes('CUOTAS_YA_GENERADAS')) {
            throw new BadRequestException(
                'Las cuotas del apartado ya fueron generadas',
            );
        }
        if (mensaje.includes('ESTADO_CUOTA_PENDIENTE_NO_EXISTE')) {
            throw new BadRequestException(
                'No existe el estado CUOTA/PENDIENTE activo',
            );
        }
